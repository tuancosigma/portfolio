...
const steps = [
  {
    ...
    title: "Collect",
    subtitle: "the signal",
    description: "Gather SIEM alerts, WAF events, firewall logs, host context, and dashboard signals from every source before triage begins.",
    code: `$ tail -f /var/log/siem/alerts.json
[wazuh] auth_failure user=admin src=10.0.4.8
[waf] rule_hit=xss path=/login
[pfsense] deny tcp 10.0.4.8:51234 -> :22`,
  },
  {
    number: "02",
    title: "Triage",
    subtitle: "the alert",
    description: "Separate noisy events from suspicious behavior with repeatable checks, clear severity, and evidence-first review.",
    code: `severity: medium
source: pfSense / Wazuh
check: user, src_ip, freq, time_window
verdict: escalate for correlation`,
  },
  {
    number: "03",
    title: "Investigate",
    subtitle: "& document",
    description: "Correlate source, timeline, and rule triggered, then write a concise handoff the next analyst can act on.",
    code: `report.write({
  timeline, evidence, confidence,
  action: 'recommended next step'
})
// ready for SOC handoff`,
  },
];
